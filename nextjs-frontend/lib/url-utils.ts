...
/**
 * Normalize a track size to canonical format: WIDTHxPITCHxLINKS
 * 
 * Rules:
 * - Strip any inch-equivalent suffixes (e.g., "16", "18", "20")
 * - Remove spaces
 * - Handle both integer and decimal values
 * - Track size format is always: WIDTH x PITCH x LINKS
 * 
 * Examples:
 * - "381x101.6x42 16"" => "381x101.6x42"
 * - "381x101.6x42 (16 inch)" => "381x101.6x42"
 * - "381x101.6x4216" => "381x101.6x42" (corrupted format fixed)
 * - "450x86x60" => "450x86x60"
 * - "400x72.5x72" => "400x72.5x72"
 */
export function normalizeTrackSize(size: string): string {
  if (!size) return "";
  
  // Remove spaces and normalize x separators
  let cleaned = size.trim().replace(/\s+/g, "").toLowerCase().replace(/x/gi, "x");
  
  // Remove parenthetical content like "(16 inch)" or "(16")"
  cleaned = cleaned.replace(/\([^)]*\)/g, "");
  
  // Remove trailing inch indicators like '16"', '16inch', etc.
  cleaned = cleaned.replace(/\d*"$/g, "").replace(/\d*inch$/gi, "");
  
  // Parse the track size: WIDTHxPITCHxLINKS
  // Handle corrupted formats where inch-equivalent was concatenated
  ...
  if (match) {
    ...
    // Fix corrupted links values where inch-equivalent was appended
    // Common patterns: 4216 (should be 42), 5080 (should be 50), 5105 (should be 51), 5588 (should be 55)
    // Links are typically 30-100, if > 100 and ends in common inch values, fix it
    const linksNum = parseInt(links, 10);
    if (linksNum > 100) {
      ...
      if (corruptedPatterns[links]) {
        links = corruptedPatterns[links];
      } else if (links.length === 4) {
        // Try to extract just the first 2 digits as links
        const potentialLinks = links.slice(0, 2);
        const potentialInch = links.slice(2);
        // Common inch equivalents are 16, 18, 20, 80, etc.
        if (["16", "18", "20", "80", "88", "05"].includes(potentialInch)) {
          links = potentialLinks;
        }
      }
    }
    
    return `${width}x${pitch}x${links}`;
  }
  
  // Return cleaned version if no match (might already be valid)
  return cleaned;
}

/**
 * Check if a string looks like a track size
 */
export function isTrackSizeFormat(value: string): boolean {
  const cleaned = value.trim().replace(/\s+/g, "").toLowerCase();
  return /^\d{2,4}x\d{2,4}\.?\d*x\d{2,4}/i.test(cleaned);
}

/**
 * Get the canonical track size URL path
 */
export function getTrackSizeUrl(size: string): string {
  return `/track-size/${normalizeTrackSize(size)}`;
}
